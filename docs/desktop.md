...
飞书在安卓、iPhone 和电脑上都能收到提醒，适合没有 iPhone 或想让几个人一起收的情况。在飞书群里打开「设置 → 群机器人 → 添加机器人」，选「自定义机器人」，安全设置选「自定义关键词」并填「有货」，然后把机器人的 webhook 地址填进「飞书机器人 Webhook」，留空表示关闭。要推到几个群，把各自的地址用分号隔开填在同一个框里，每个地址单独推送、单独报错。
...
点击「测试提醒」验证。推送失败只显示渠道错误，不改变库存判定。webhook 地址里带着机器人的密钥，分享日志和截图时请隐去。
