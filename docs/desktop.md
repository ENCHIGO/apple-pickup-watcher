...
选择地区、品类、门店和型号，添加目标后点击「开始」。门店可以一次勾选多家（下拉里有「全选 / 清空」），点「添加」会给每家店各加一条同型号的目标，已在列表里的跳过；添加后型号清空、门店保留，方便接着给同一批门店加下一个型号。iPhone 按机型、容量、颜色逐步选择，其他品类直接选择完整型号。同城门店合并成一次请求，多选不增加请求量。
