...
- **代理线路**：`check` / `watch` 的 `--proxy <URL>`（可重复或逗号分隔；`http` / `https` / `socks5` / `socks5h`，可带账号密码）给进程多加几条出口线路。每条线路各有自己的 cookie 罐、冷却和令牌桶，请求在线路之间轮流走；某条被拦（541）同一轮就换下一条，代理连不上也换下一条。桌面版在设置里填「代理地址」，效果相同。配额按出口 IP 计，多一条线路多一份配额。
