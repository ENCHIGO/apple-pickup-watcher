...
Apple 的边缘节点对取货接口按**出口 IP** 计数（2026-09-14 实测，见 issue #3）：不间断发到约 30 次就返回 HTTP 541，之后十几分钟内怎么发都是 541，停下来额度才慢慢恢复。带不带 cookie、请求头像不像浏览器、TLS 指纹是不是 Chrome，都不影响这个计数。所以从 v0.4.2 起客户端做两件事：
