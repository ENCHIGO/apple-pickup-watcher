...
在 iPhone 上装 [Bark](https://github.com/Finb/Bark)，把它给你的推送地址填进设置里的「Bark 推送地址」，点「测试提醒」确认整条链路通畅。几台手机都要收就把地址用分号隔开一起填。安卓手机或想在飞书群里收，就给群添加一个自定义机器人，安全设置选关键词「有货」，把 webhook 地址填进「飞书机器人 Webhook」，步骤见[桌面版指南](docs/desktop.md#飞书群机器人推送)。推送失败不会改变库存判定。
