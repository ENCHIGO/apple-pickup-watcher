...
`force push` 那条连管理员也挡，且**不受 `enforce_admins` 影响**，一开始就是硬的。所以
写坏了的提交没法靠改历史抹掉，只能靠新提交纠正 —— main 上的 `4d570e2`
（「临时验证分支保护，不会留下」）就是这么留下的：那是一次验证保护规则是否生效的空提交，
内容树和上一个提交完全相同（都是 `23ff286`），一个字节都没改，那句「不会留下」当时写错了。
...
它会依次查询七个地区的真实门店，检查三件事：接口还在不在、响应结构有没有变、
`pickupDisplay` 有没有出现我们不认识的新取值。
